...
These certificates can be overridden via any of the following methods, in order of precedence:

- Set them directly via the CLI option `--ca-certificates <path>`.
- Specify a custom path in the `pkl:settings` module using `http.cacerts`.
- Add them to a directory at path `~/.pkl/cacerts/`.

All these options will *replace* the default CA certificates bundled with Pkl. +
